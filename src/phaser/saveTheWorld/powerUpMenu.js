var PhaserGameObject = (function () {
    ...
    PhaserGameObject.prototype.init = function (el, parent, options) {
        ...
        function create() {
            ...
            phaserGroup.addMany(8, [instructionbox, headerbox]);
            phaserMaster.let('columns', 3);
            var desiredColumns = 4;
            var rows = 0;
            var columns = 0;
            weaponList.forEach(function (weapon, index) {
                var sprite = phaserSprites.add({ x: 0, y: game.canvas.height - instructions.height - padding, name: "powerUp_" + index, reference: "" + weapon.sprite });
                var centerX = (game.canvas.width / desiredColumns) * (columns) + ((game.canvas.width / desiredColumns) / 2) - (sprite.width / 2);
                sprite.x = centerX;
