var PhaserGameObject = (function () {
    ...
    PhaserGameObject.prototype.init = function (el, parent, options) {
        ...
        function create() {
            ...
            var weaponList = [];
            for (var weapon in weaponData.primaryWeapons) {
                weaponList.push(weaponData.primaryWeapons[weapon]);
            }
            for (var weapon in weaponData.secondaryWeapons) {
                weaponList.push(weaponData.secondaryWeapons[weapon]);
            }
            phaserMaster.let('weaponList', weaponList);
            var overlaybmd = phaserBmd.addGradient({ name: 'overlaybmd', start: '#2f2f2f', end: '#2f2f2f', width: 5, height: 5, render: false });
            ...
            var popupBox = phaserSprites.add({ name: "popupBox", group: 'popupBoxGroup', reference: phaserBmd.get('bgGradient').cacheBitmapData, visible: false });
            popupBox.popUp = function (message, callback) {
                var _this = this;
                if (callback === void 0) { callback = function () { }; }
                var padding = 50;
                var popupText = phaserTexts.add({ name: 'popupText', group: 'popupBoxGroup', font: 'gem', size: 18, default: "" + message, visible: true });
                ...
                this.game.add.tween(this).to({ y: this.y + 20, alpha: 1 }, 100, Phaser.Easing.Linear.In, true, 0, 0, false).
                    ...
                    _this.game.add.tween(popupText).to({ alpha: 1 }, 100, Phaser.Easing.Linear.In, true, 0, 0, false).
                        onComplete.add(function () {
                        callback();
                    });
                });
            };
            popupBox.popOut = function (callback) {
                var _this = this;
                if (callback === void 0) { callback = function () { }; }
                var popupText = phaserTexts.get('popupText');
                this.game.add.tween(popupText).to({ alpha: 0 }, 100, Phaser.Easing.Linear.Out, true, 0, 0, false).
                    onComplete.add(function () {
                    phaserTexts.destroy('popupText');
                    _this.game.add.tween(_this).to({ alpha: 0 }, 100, Phaser.Easing.Linear.Out, true, 0, 0, false).
                        onComplete.add(function () {
                        callback();
                    });
                });
            ...
            weaponList.forEach(function (weapon, index) {
                ...
                var equippedSprite = phaserSprites.add({ name: "equippedIcon_" + index, group: 'e', reference: "equipped", visible: false });
                ...
                var purchasedSprites = phaserSprites.add({ name: "purchasedIcon_" + index, group: 'p', reference: "purchased", visible: false });
        ...
        function checkForEquipped() {
            ...
            var gameData = phaserMaster.get('gameData');
            phaserSprites.getGroup('e').forEach(function (item) {
                item.visible = false;
            });
            weaponList.forEach(function (item, index) {
        ...
        function checkForPurchases() {
            var weaponList = phaserMaster.get('weaponList');
            var gameData = phaserMaster.get('gameData');
            var purchaseHistory = phaserMaster.get('gameData').purchaseHistory;
            weaponList.forEach(function (item, index) {
                item.purchased = false;
                item.canPurchase = (gameData.money >= item.price);
                for (var _i = 0, purchaseHistory_1 = purchaseHistory; _i < purchaseHistory_1.length; _i++) {
                    var id = purchaseHistory_1[_i];
                    if (item.id === id) {
                        item.purchased = true;
        ...
        function updateDescription(index) {
            var weaponList = phaserMaster.get('weaponList');
            var descriptionText = phaserTexts.get('instructions');
        ...
        function updateColumn(inc) {
            var selected = phaserMaster.get('selected');
            var weaponsList = phaserMaster.get('weaponList');
            selected += inc;
        ...
        function popupMessage(message, duration) {
            ...
            var popupBox = phaserSprites.get("popupBox");
            popupBox.popUp(message);
            setTimeout(function () {
                popupBox.popOut(function () {
                    phaserMaster.changeState('READY');
                });
            }, duration);
        }
        function makePurchaseOrEquip() {
            var selected = phaserMaster.get('selected');
            var weaponsList = phaserMaster.get('weaponList');
            var selectedItem = weaponsList[selected];
            var gameData = phaserMaster.get('gameData');
